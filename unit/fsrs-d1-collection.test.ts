...
describe("D1FsrsCollection", () => {
  ...
  test("findOne honours the same-origin $regex, composed with $nin and $not", async () => {
    // sameOriginQuery() layers `$regex` onto the url constraints. This matcher
    // drops any operator it does not know, so without explicit support the
    // "prefer this origin" query would quietly return the whole deck — on D1
    ...
    // Drained: nothing on that origin is due and not excluded.
