...
function extractStorageClass() {
  // The class calls originOf() for the same-origin preference; lift the real
  // one rather than injecting a hand-written copy the bundle might not match.
  return `${extractBlock("function originOf(")}\n${extractBlock("class GMCardStorage {")}`;
}
...
describe("soft delete", () => {
  ...
  it("serves the origin you are on first, then most recently due", () => {
    ...
    // with the origin drained, the rest of the deck follows in the usual order.
    const { cs } = makeStorage();
    ...
    expect(urls(cs.getDueCards(10, "https://www.youtube.com"))).toEqual([LATER, STALE, FRESH]);
    // Exact: a subdomain or another scheme is a different origin.
    expect(urls(cs.getDueCards(10, "https://m.youtube.com"))).toEqual([FRESH, LATER, STALE]);
    expect(urls(cs.getDueCards(10, "http://www.youtube.com"))).toEqual([FRESH, LATER, STALE]);

    // No preference, or an origin with nothing due: plain most-recent-first.
    expect(urls(cs.getDueCards(10))).toEqual([FRESH, LATER, STALE]);
    expect(urls(cs.getDueCards(10, "https://nothing.test"))).toEqual([FRESH, LATER, STALE]);
  });
